...
describe("FastMCP OAuth Support", () => {
  it("should serve OAuth authorization server metadata", async () => {
    ...
    try {
      ...
      expect(metadata.dpop_signing_alg_values_supported).toEqual(["ES256", "RS256"])
    } finally {
      await server.stop()
    }
  })
  ...
  it("should serve OAuth protected resource metadata", async () => {
    ...
    try {
      ...
      expect(metadata.jwks_uri).toBe("https://test-server.example.com/.well-known/jwks.json")
      expect(metadata.bearer_methods_supported).toEqual(["header"])
      expect(metadata.resource_documentation).toBe("https://docs.example.com/api")

      // New fields added for RFC 9728 compliance
      expect(metadata.authorization_details_types_supported).toEqual(["payment_initiation"])
      expect(metadata.dpop_bound_access_tokens_required).toBe(true)
      expect(metadata.dpop_signing_alg_values_supported).toEqual(["ES256", "RS256"])
      expect(metadata.resource_name).toBe("Test API")
      expect(metadata.resource_policy_uri).toBe("https://test-server.example.com/policy")
      expect(metadata.resource_signing_alg_values_supported).toEqual(["RS256"])
      expect(metadata.resource_tos_uri).toBe("https://test-server.example.com/tos")
      expect(metadata.scopes_supported).toEqual(["read", "write", "admin"])
      expect(metadata.service_documentation).toBe("https://developer.example.com/api")
  ...
  it("should return 404 for OAuth endpoints when disabled", async () => {
    ...
    try {
      const authServerResponse = await fetch(`http://localhost:${port}/.well-known/oauth-authorization-server`)
      expect(authServerResponse.status).toBe(404)

      const protectedResourceResponse = await fetch(`http://localhost:${port}/.well-known/oauth-protected-resource`)
