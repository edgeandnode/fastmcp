...
type InputResourceTemplate<
  ...
  Arguments extends InputResourceTemplateArgument<T>[] = InputResourceTemplateArgument<T>[],
> = {
  arguments: Arguments
  description?: string
...
type PromptArgumentsToObject<T extends { name: string; required?: boolean }[]> = {
  [K in T[number]["name"]]: Extract<T[number], { name: K }>["required"] extends true ? string : string | undefined
}
...
export class FastMCPSession<T extends FastMCPSessionAuth = FastMCPSessionAuth> extends FastMCPSessionEventEmitter {
  ...
  private setupPromptHandlers(prompts: Prompt<T>[]) {
    ...
    this.#server.setRequestHandler(GetPromptRequestSchema, async (request) => {
      ...
      try {
        result = await prompt.load(args as Record<string, string | undefined>, this.#auth)
      } catch (error) {
  ...
  private setupResourceTemplateHandlers(resourceTemplates: ResourceTemplate<T>[]) {
 ...
 */
function convertObjectToSnakeCase(obj: Record<string, unknown>): Record<string, unknown> {
...
export class FastMCP<T extends FastMCPSessionAuth = FastMCPSessionAuth> extends FastMCPEventEmitter {
   ...
   */
  public addPrompt<const Args extends InputPromptArgument<T>[]>(prompt: InputPrompt<T, Args>) {
    this.#prompts.push(prompt)
  }
   ...
   */
  public addResourceTemplate<const Args extends InputResourceTemplateArgument[]>(
  ...
  public async embedded(uri: string): Promise<ResourceContent["resource"]> {
    ...
    for (const template of this.#resourcesTemplates) {
      ...
      const result = await template.load(params as ResourceTemplateArgumentsToObject<typeof template.arguments>)
